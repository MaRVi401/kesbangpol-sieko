...
@section('content')
    <div class="p-4 mt-14">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-500">Aktivasi
                            Pemohon</span>
                    </div>
                </li>
            </ol>
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Tinjau data diri dan dokumen pendukung pemohon sebelum
                memberikan akses sistem.</p>
        </div>
                        ...
                        @forelse($pendingUsers as $user)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <button data-modal-target="modal-{{ $user->uuid }}"
                                        data-modal-toggle="modal-{{ $user->uuid }}"
                                    ...
                                    <div id="modal-{{ $user->uuid }}" tabindex="-1" aria-hidden="true"
                                        ...
                                        <div class="relative p-4 w-full max-w-2xl max-h-full text-left">
                                            <div
                                                class="relative bg-white rounded-xl shadow-lg dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                                                <div
                                                    ...
                                                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">Detail
                                                        Pengajuan: {{ $user->nama }}</h3>
                                                    <button type="button"
                                                        class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                        data-modal-hide="modal-{{ $user->uuid }}">
                                                ...
                                                <div class="p-6 space-y-6">
                                                    <div class="grid grid-cols-2 gap-4">
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                                            <p class="text-xs text-gray-500 uppercase font-bold mb-1">NIK
                                                                Ketua</p>
                                                            <p class="text-sm text-gray-900 dark:text-white">
                                                                {{ $user->pemohon->nik_ketua }}</p>
                                                        </div>
                                                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                                                            <p class="text-xs text-gray-500 uppercase font-bold mb-1">Nama
                                                                Organisasi</p>
                                                            <p class="text-sm text-gray-900 dark:text-white">
                                                                {{ $user->pemohon->nama_organisasi ?? '-' }}</p>
                                                        </div>
                                                        <div
                                                            class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg col-span-2">
                                                    ...
                                                    <div class="space-y-3">
                                                        <p class="text-sm font-bold text-gray-900 dark:text-white">Preview
                                                            KTP Ketua:</p>

                                                        {{-- Tambahkan pengecekan if untuk kta_path --}}
                                                        @if (!empty($user->pemohon->kta_path))
                                                            <img src="{{ route('file.show', ['path' => $user->pemohon->kta_path]) }}"
                                                                class="w-full h-48 object-cover rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm"
                                                                alt="KTP Preview">
                                                        @else
                                                            <div
                                                                class="w-full h-48 flex items-center justify-center bg-gray-100 rounded-xl border border-gray-200 dark:bg-gray-700 dark:border-gray-600">
                                                                <span class="text-sm text-gray-500 dark:text-gray-400">KTP
                                                                    belum diunggah</span>
                                                            </div>
                                                        @endif
                                                        ...
                                                        <div class="pt-2">
                                                            ...
                                                            {{-- Tambahkan pengecekan if untuk surat_rekomendasi_path --}}
                                                            @if (!empty($user->pemohon->surat_rekomendasi_path))
                                                                <a href="{{ route('file.show', ['path' => $user->pemohon->surat_rekomendasi_path]) }}"
                                                                    target="_blank"
                                                            ...
                                                            @else
                                                                <p class="text-sm text-gray-500 italic dark:text-gray-400">
                                                                    Dokumen belum diunggah</p>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                                                <div
                                                    class="flex items-center justify-end p-6 space-x-3 border-t dark:border-gray-700">
                                                    <form action="{{ route('user-management.activate', $user->uuid) }}"
                                                        method="POST">
                                                        @csrf
                                                        <input type="hidden" name="status" value="ditolak">
                                                        <button type="submit"
                                                            class="px-5 py-2.5 text-sm font-medium text-red-600 bg-white border border-red-200 rounded-lg hover:bg-red-50 focus:ring-4 focus:ring-red-100 transition-all">Tolak
                                                            Pengajuan</button>
                                                    </form>
                                                    <form action="{{ route('user-management.activate', $user->uuid) }}"
                                                        method="POST">
                                                        ...
                                                        <button type="submit"
                                                            class="px-8 py-2.5 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 shadow-md transition-all">Aktifkan
                                                            Akun</button>
                                                    </form>
                                                </div>
                                            </div>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-500 dark:text-gray-400">Tidak ada pemohon yang menunggu
                                            aktivasi.</p>
                                    </div>
                                </td>
                            </tr>
